...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=CustomUser
        fields=['id', 'username', 'last_name', 'first_name', 'profile_image', 'groups', 'is_auth', 'location', 'birth_year', 'number_phone', 'facebook', 'tiktok', 'email']
...
class CustomUserTopSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=CustomUser
        fields=['id', 'email', 'username', 'last_name', 'first_name','profile_image', 'groups', 'user_permissions', 'is_auth', 'total_replies']
...
class CustomUserAboutSerializer(serializers.ModelSerializer):
    ...
    def get_total_replies(self, obj):
        ...
        total_replies_thread = Thread.objects.filter(user=obj).aggregate(total_replies=Sum('replies'))['total_replies'] or 0
        return total_replies_thread

    def get_total_views(self, obj):
        total_views_article = Articles.objects.filter(user=obj).aggregate(total_views=Sum('num_views'))['total_views'] or 0
    ...
    class Meta:
        ...
        fields = ['id', 'email', 'username', 'last_name', 'first_name', 'profile_image', 'user_permissions', 'is_auth',
                  'total_articles', 'total_threads', 'total_likes', 'total_replies', 'total_views', 'location', 'birth_year', 'number_phone', 'facebook', 'tiktok']